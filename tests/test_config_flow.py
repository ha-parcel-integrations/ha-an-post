"""Tests for the An Post config and options flow."""
...
@pytest.mark.parametrize(
    "error,expected",
    [
        (AnPostAuthError("HTTP 401"), "invalid_auth"),
        (AnPostApiError("HTTP 500"), "cannot_connect"),
...
async def test_reauth_surfaces_invalid_credentials(hass):
    ...
    result = await entry.start_reauth_flow(hass)
    with patch(LOGIN, new=AsyncMock(side_effect=AnPostAuthError("HTTP 401"))):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], CREDENTIALS
        )
...
async def test_reauth_surfaces_connection_errors(hass):
    ...
    result = await entry.start_reauth_flow(hass)
    with patch(LOGIN, new=AsyncMock(side_effect=AnPostApiError("HTTP 500"))):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], CREDENTIALS
        )
