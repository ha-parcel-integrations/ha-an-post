...
# no planned_from at all); mid = anything else still in flight. This is an
# account-based coordinator (Section 2.2), so it never
# fully stops — the mid-tier poll is also how a new shipment gets discovered,
# since a single account call is the only way to see one that appeared
# without going through this integration.
