"""Sensor platform for the An Post parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: AnPostConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up An Post sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        AnPostIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            AnPostParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(AnPostNextDeliverySensor(coordinator, entry))
    entities.append(AnPostDeliveredParcelsSensor(coordinator, entry))
    entities.append(AnPostLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class AnPostIncomingParcelsSensor(
    CoordinatorEntity[AnPostCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                AnPostParcelSensor(self.coordinator, self._entry, barcode)
...
class AnPostParcelSensor(CoordinatorEntity[AnPostCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked An Post parcel."""
    ...
    def __init__(
        self, coordinator: AnPostCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class AnPostNextDeliverySensor(
    CoordinatorEntity[AnPostCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: AnPostCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class AnPostDeliveredParcelsSensor(
    CoordinatorEntity[AnPostCoordinator], SensorEntity
):
    """Recently delivered tracked An Post parcels."""
...
class AnPostLastUpdateSensor(
    CoordinatorEntity[AnPostCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when An Post was last polled successfully."""
